/* ================================================================
   app.js — Uplift Compliment Generator
   ================================================================ */
...
const CATEGORY_META = {
  encouragement: { emoji: "🌟", label: "Encouragement", pillClass: "pill-encouragement" },
  professional:  { emoji: "💼", label: "Professional",  pillClass: "pill-professional" },
  humor:         { emoji: "😄", label: "Humor",         pillClass: "pill-humor" },
  kindness:      { emoji: "💛", label: "Kindness",      pillClass: "pill-kindness" },
};
...
Object.keys(COMPLIMENTS).forEach((k) => (state.seen[k] = new Set()));
...
(function initParticles() {
  ...
  function draw() {
    ...
    for (const p of particles) {
      ...
      if (p.y < -10) p.y = h + 10;
      if (p.y > h + 10) p.y = -10;
      const alpha = p.baseAlpha + Math.sin(p.phase) * 0.07;
      ctx.beginPath();
...
function pickCompliment() {
  ...
  if (cat === "all") {
    Object.entries(COMPLIMENTS).forEach(([key, arr]) => {
      arr.forEach((text, i) => {
        if (!state.seen[key].has(i)) {
          pool.push({ text, category: key, index: i });
        }
      });
    });
    if (pool.length === 0) {
    ...
    pool = COMPLIMENTS[cat]
      .map((text, i) => ({ text, category: cat, index: i }))
      .filter((c) => !state.seen[cat].has(c.index));
    if (pool.length === 0) {
      state.seen[cat] = new Set();
      showToast("All seen — reshuffled!");
      return pickCompliment();
    }
  }
  ...
  state.seen[pick.category].add(pick.index);
  return pick;
}
...
function displayCompliment() {
  const pick = pickCompliment();
  const text = personalize(pick.text);
  const meta = CATEGORY_META[pick.category];

  dom.compliment.classList.add("exit");

  setTimeout(() => {
    state.currentCompliment = pick.text;
    state.currentComplimentCat = pick.category;
    state.hasGenerated = true;

    dom.compliment.textContent = `\u201C ${text} \u201D`;

    dom.pill.className = `category-pill ${meta.pillClass}`;
    dom.pill.innerHTML = `<span class="pill-dot"></span><span class="pill-label">${meta.label}</span>`;
...
function syncFavBtn() {
  const isFav = state.favorites.some((f) => f.text === state.currentCompliment);
...
function renderFavorites() {
  if (!state.favorites.length) {
    dom.modalBody.innerHTML = `
      <div class="empty-state">
        ...
        <p>No favorites yet.<br>Tap the star on compliments you love!</p>
      </div>`;
    return;
  }
  ...
  state.favorites.forEach((f, i) => {
    ...
    li.innerHTML = `
      <div>
        <p>${f.text}</p>
        <div class="fav-cat">${CATEGORY_META[f.category].emoji} ${CATEGORY_META[f.category].label}</div>
      </div>
...
dom.categorySelect.addEventListener("change", () => {
  state.currentCategory = dom.categorySelect.value;
});

dom.copyBtn.addEventListener("click", async () => {
  if (!state.hasGenerated) return;
  const text = dom.compliment.textContent;
  try {
...
dom.favBtn.addEventListener("click", (e) => {
  if (!state.hasGenerated) return;
  const idx = state.favorites.findIndex((f) => f.text === state.currentCompliment);
  if (idx > -1) {
    state.favorites.splice(idx, 1);
    showToast("Removed from favorites");
  } else {
    state.favorites.push({
      text: state.currentCompliment,
      category: state.currentComplimentCat,
    });
